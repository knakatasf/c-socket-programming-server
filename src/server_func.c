...
/*
 *  Creates a welcoming socket, specifying the server's port.
 *  Waits for a client request.
 *  Upon acceptance of the client request, pass the client socket. 
 */
struct socket_pair open_client_tcp_socket(const int server_port)
{
    ...
    socks.client_sock = accept(socks.server_sock, (struct sockaddr*)&addr, &addr_len); // Populates client's addr and return the client socket.
...
long receive_packet_trains(const int server_port, const int num_of_packets, const int packet_size)
{
    ...
    int receive_buffer_size = 256 * 1024 * 1024; // 256 MB
    if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF, &receive_buffer_size, sizeof(receive_buffer_size)) < 0) // Sets the socket size enough to receive the two trains.
    {
        perror("Failed to set receive buffer size");
    }

    /*
     *  recvfrom() will block until receiving a packet.
     *  So, we want to set the socket's timeout.
    ...
    if (bind(sock, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) // We need to specify the server's port for the trains.
    {
        perror("Bind failed");
        close(sock);
        exit(EXIT_FAILURE);
    ...
    char* buffer = malloc(num_of_packets * packet_size * 8); // Allocates enough buffer to receive the trains.
    if (!buffer) {
    ...
    long first_train_time_duration = receive_packet_train(sock, buffer, packet_size, &client_addr);
    long second_train_time_duration = receive_packet_train(sock, buffer, packet_size, &client_addr);

    free(buffer);
    
    close(sock);
    printf("Finished receiving packet train\n");

    if (first_train_time_duration != -1 && second_train_time_duration != -1) // Return the result only when the server receives BOTH trains.
    {
